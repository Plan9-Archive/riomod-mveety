...
filsysinit(Channel *cxfidalloc)
{
	...
	if(cexecpipe(&p0, &wctlfd) < 0)
		goto Rescue;
	sprint(srvwctl, "/srv/riomodwctl.%s.%d", fs->user, pid);
	 ...
	 * Post srv pipe
	 */
	sprint(srvpipe, "/srv/riomod.%s.%d", fs->user, pid);
...
filsysproc(void *arg)
{
	...
	for(;;){
		...
		if(n <= 0){
			...
			fprint(2, "riomod: %d: read9pmsg: %d %r\n", getpid(), n);
			errorshouldabort = 0;
...
filsysauth(Filsys *fs, Xfid *x, Fid*)
{
	Fcall t;
		...
		return filsysrespond(fs, x, &t, "riomod: authentication not required");
}
