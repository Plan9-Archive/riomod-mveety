...
char *acmeargv[] = { "acme", nil };
char *vtargv[] = {"vt", "-xb", nil};
...
usage(void)
{
	fprint(2, "usage: riomod [-f font] [-i initcmd] [-k kbdcmd] [-s] [-b/w] [-d] [-n]\n");
	exits("usage");
}
...
threadmain(int argc, char *argv[])
{
	...
	dprint("riomod by matthew veety");
	...
	if(access(fontname, 0) < 0){
		fprint(2, "riomod: can't access %s: %r\n", fontname);
		exits("font open");
	}
	...
	if(geninitdraw(nil, derror, nil, "riomod", nil, Refnone) < 0){
		fprint(2, "riomod: can't open display: %r\n");
		exits("display open");
	}
	iconinit();
	view = screen;
	...
	if(filsys == nil)
		fprint(2, "riomod: can't create file system server: %r\n");
	else{
		...
		if(initstr)
			...
			if (loadrc == TRUE)
				proccreate(initcmd, strcat(homedir,"/lib/riomod-start"), STACK);
			else
				dprint("not loading rcfile");
		if(kbdin){
			kbdargv[2] = kbdin;
			r = screen->r;
			r.max.x = r.min.x+300;
...
void
button3menu(Window *w)
{
	int i;
	
	dprint("button 3 menu");
	...
	if(w != 0) {
		if(w->scrolling)
			menu3str[Scroll] = "scroll off";
		else
			menu3str[Scroll] = "scroll on";
	} else {
		menu3str[Scroll] = "no scrolling";
	}
	...
	case Hide:
		...
		break;
	case Acme:
		new(sweep(), FALSE, scrolling, 0, nil, "/bin/acme", acmeargv);
		break;
	case Exit:
		if(Hidden > Exit){
			send(exitchan, nil);
...
button2menu(void)
{
	dprint("button 2 menu");
	int i;
	for(i=0; i<nhidden; i++)
		menu2str[i] = hidden[i]->label;
	menu2str[i] = nil;
...
sweep(void)
{
	...
	while(mouse->buttons == 4){
		...
		if(!eqpt(mouse->xy, p)){
			...
			if(Dx(r)>5 && Dy(r)>5){
				i = allocwindow(wscreen, r, Refnone, 0xBBBBBBFF); /* grey */
				freeimage(oi);
				if(i == nil)
					goto Rescue;
				oi = i;
